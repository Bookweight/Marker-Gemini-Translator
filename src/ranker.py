...
class PaperRanker:
    ...
    # --- Ranking Logic ---
    def rank_candidates(self, papers: List[Dict[str, Any]], top_k: int = 5) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_score(self, paper: Dict[str, Any]) -> tuple[float, bool]:
        ...
        # C. 取得影響力引用數 (加上時效性紅利)
        raw_citations = paper.get('influentialCitationCount') or paper.get('citationCount', 0)
        ...
        if year:
            try:
                ...
                bonus_base = self.config.get('ranking', {}).get('recency_bonus_baseline', 50)
