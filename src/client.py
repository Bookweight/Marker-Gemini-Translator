...
class S2Client:
    ...
    def __init__(self, api_key: Optional[str], config: Dict[str, Any]):
        ...
        if self.api_key:
            ...
            self.logger.warning("未偵測到 API Key，將以未認證模式運作 (Rate Limit 極低)")
    ...
    def _make_request(self, method: str, endpoint: str, params: Optional[Dict] = None, json_data: Optional[Dict] = None) -> Any:
        ...
        try:
            ...
            if response.status_code == 429:
                self.logger.warning(f"⚠️ 觸發 Rate Limit (429)，Tenacity 將接手重試...")
    ...
    def get_batch_details(self, paper_ids: List[str]) -> List[Dict[str, Any]]:
        ...
        self.logger.info(f"📥 開始批量下載詳情: {len(paper_ids)} 篇論文，分 {total_batches} 批次處理")
        ...
        for i in range(0, len(paper_ids), self.batch_size):
            ...
            try:
                self.logger.debug(f"處理批次 {i // self.batch_size + 1}/{total_batches} (Size: {len(chunk)})")
                result = self._make_request('POST', endpoint, params=params, json_data=payload)
